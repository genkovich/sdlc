...
small change, one [`sdlc:reviewer`](../../../agents/reviewer.md) pass covers them all; for a large
...
The `sdlc:reviewer` agent has read-only tools (`Read`, `Grep`, `Glob`, `Bash`), re-reads
...
Per [`../../_shared/agent-roster.md`](../../_shared/agent-roster.md): `sdlc:reviewer` runs
`model: opus` / `effort: high`; the dispatch call is `subagent_type: "sdlc:reviewer"`.
